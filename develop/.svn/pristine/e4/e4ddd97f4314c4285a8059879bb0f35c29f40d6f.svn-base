...
- (void)loadEmotionView
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        for (NSInteger i = 0; i < emoticonsArr.count; i ++) {
            ...
            emotionManager.emotionIcon = [emoticonDic objectForKey:@"chat_icon"];
            
            NSArray * emoticonItemsArr = [emoticon_item_list_model getAllItemWithEmoticonID:[[emoticonDic objectForKey:@"emoticon_id"] longLongValue]];
            NSMutableArray *emoticons = [NSMutableArray array];
            for (NSInteger j = 0; j < emoticonItemsArr.count; j ++) {
//                XHEmotion *emotion = [[XHEmotion alloc] init];
//                NSString *imageName = [NSString stringWithFormat:@"section0_emotion%ld", (long)j % 16];
//                emotion.emotionPath = [[NSBundle mainBundle] pathForResource:[NSString stringWithFormat:@"test_gif.gif"] ofType:@""];
...
//- (NSMutableArray *)reorderEmoticonArray:(NSArray *)arr
//{
//    NSInteger timeNumbers = arr.count /8;
//}
...
- (void)loadInputBarAndMessageTable
{
    ...
    // 设置整体背景颜色
    messageTableView.backgroundColor = [[ThemeManager shareInstance]getColorWithName:@"COLOR_LIGHTWEIGHT"];
