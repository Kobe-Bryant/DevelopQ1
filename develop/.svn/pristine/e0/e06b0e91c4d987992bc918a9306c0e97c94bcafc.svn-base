    ...
    if (_scrollView) {
        for (int i = 0; i < _photos.count; i++) {
            ...
            if ([[_photos lastObject] isKindOfClass:[NSString class]]) {
                ...
                [imagev setImageWithURL:[NSURL URLWithString:[_photos objectAtIndex:i]] placeholderImage:IMG(@"default_600") completed:^(UIImage* image,NSError* error,SDImageCacheType SDImageCacheTypeNone){
                    if (image) {
                    }else{
        ...
        _labIndicate.text = [NSString stringWithFormat:@"%d / %d",_currentIndex + 1,_photos.count];
...
//    [Common checkProgressHUD:@"保存成功!" andImage:nil showInView:self];
    [Common checkProgressHUDShowInAppKeyWindow:@"保存成功!" andImage:KAccessSuccessIMG];
}
