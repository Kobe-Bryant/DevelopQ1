...
{
//    保存到当前的缓存里面，是不可以的，当版本的升级的时候，会把当前缓存中的数据清除掉  add vincent
//	NSArray *paths = [[NSArray alloc] initWithArray:NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES)]; NSDocumentDirectory
	...
	RELEASE_SAFE(paths);
	RELEASE_SAFE(documentsDirectory);
	return filepath;
}
...
+ (NSString *)getUserDBPath
{
    ...
    NSNumber * currentOrgId = [whole_users_model getPreviousOrgId];
    
    NSString * userDBFileName = nil;
    NSString * dbFilePath = nil;
    if (userAccountName != nil && currentOrgId != nil) {
        userDBFileName = [NSString stringWithFormat:@"%@_%d.db",userAccountName,currentOrgId.intValue];
        dbFilePath = [FileManager getFileDBPath:userDBFileName];
    }
    return dbFilePath;
}
