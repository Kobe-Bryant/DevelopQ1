...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _separatorLine.backgroundColor = [UIColor colorWithRed:241/255.0f green:241/255.0f blue:241/255.0f alpha:1];
...
- (void)layoutSubviews
{
    ...
    self.separatorLine.frame = (CGRect){CGPointMake(0, 45), CGSizeMake(self.bounds.size.width, 1.5)};
    
    [self layoutContentScrollView];
    self.contentScrollView.frame = (CGRect){CGPointMake(0, self.titleLabel.frame.size.height + 15), self.contentScrollView.bounds.size};
...
//    self.cancelButton.frame = CGRectMake(self.bounds.size.width*0.05, self.titleLabel.bounds.size.height + self.contentScrollView.bounds.size.height, self.bounds.size.width*0.9, 44);
    ...
    self.cancelButton.frame = CGRectMake(self.bounds.size.width - 80, 2, 80, 44);
...
- (void)layoutContentScrollView
{
    ...
    CGSize itemSize = CGSizeMake((self.bounds.size.width - margin.left - margin.right) / 3, 95);
