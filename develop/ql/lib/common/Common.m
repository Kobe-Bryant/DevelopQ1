...
+ (NSString *)makeFriendTime:(int)times
{
    ...
    int countTimes = (int)(currentTime - times);
    //一分钟秒数
    int oneM = 60;
    //一小时秒数
    int oneH = 60 * oneM;
...
+ (NSString *)makeMessageListHumanizedTimeForWithTime:(double)theTime
{
    ...
    {//同周昨天之外显示星期
        formatter.locale = [Global currentLocale];
        [formatter setDateFormat:@"EEEE"];
    } else if (theDay == currentDay -1)
    {//昨天的就显示昨天
        if ([Global isChineseLanguage]) {
            return @"昨天";
        } else {
            return @"Yesterday";
        }
...
JudgeMethodBlack judgeMethod = ^(NSArray * theArr, ParseMethod theMethod){
    if (![[theArr lastObject]isEqual:@"cwRequestFail"] && ![[theArr lastObject]isEqual:@"cwRequestTimeout"] && theArr != nil && [[theArr firstObject]objectForKey:@"Error"] == nil) {
        theMethod();
    } else {
...
+ (void)securelyparseHttpResultArr:(NSArray *)resultArray andMethod:(ParseMethod)method
{
    judgeMethod(resultArray,method);
}
